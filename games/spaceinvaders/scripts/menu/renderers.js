...
var mapMargin = 32 // 4*tileSize; // margin between the map and the screen
var mapPad = 1 // tileSize/8; // padding between the map and its clipping

var mapWidth = screenWidth-mapMargin*2-mapPad*2 // 28*tileSize+mapPad*2;
var mapHeight = screenHeight-mapMargin*2-mapPad*2 // 36*tileSize+mapPad*2;
...
var initRenderer = function(){
    ...
    // drawing scale
    var scale = 1;        // scale everything by this amount
    ...
    CommonRenderer.prototype = {
        ...
        drawMessage: function(text, color, x,y) {
            ...
            var l = text.length+2;
            ctx.fillStyle = "#000";
            ctx.fillRect((x-1)*menuSize, (y-1)*menuSize, l*menuSize, 3*menuSize);
            ctx.fillStyle = color;
            ctx.fillText(text, x*menuSize, y*menuSize);
        },

        drawReadyMessage: function() {
            this.drawMessage("READY !!!","#FF0",13,20);
            // drawExclamationPoint(ctx,16*menuSize+3, 20*menuSize+3);
        },
