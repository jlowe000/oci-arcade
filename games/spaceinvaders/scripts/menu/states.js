...
var homeState = (function(){
    ...
    var menu = new Menu("WELCOME",2*menuSize,0*menuSize,mapWidth-4*menuSize,3*menuSize,menuSize,menuSize+"px ArcadeR", "#EEE");
    menu.addTextButton("OCI INVADERS",
        function() {
            gameMode = GAME_INVADERS;
            exitTo(newGameState);
        });
    // menu.addSpacer(0.5);
    menu.addTextButton("SCORE BOARD",
        function() {
...
var learnState = (function(){
    ...
    var menu = new Menu("LEARN", 2*menuSize,-menuSize,mapWidth-4*menuSize,3*menuSize,menuSize,menuSize+"px ArcadeR", "#EEE");
    menu.addSpacer(7);
    ...
    return {
        init: function() {
            ...
            blinky.setPos(14*menuSize-1, 13*menuSize+midTile.y);
            pinky.setPos(15*menuSize+midTile.x, 13*menuSize+midTile.y);
            inky.setPos(9*menuSize+midTile.x, 16*menuSize+midTile.y);
            clyde.setPos(18*menuSize+midTile.x, 16*menuSize+midTile.y);

            // set pacman state
            pacman.reset();
            pacman.setPos(14*menuSize-1,22*menuSize+midTile.y);
    ...
    return {
        ...
        draw: function() {
            ...
            renderer.renderFunc(function(ctx){
                ctx.font = menuSize+"px ArcadeR";
                ctx.fillStyle = nameColor;
                ctx.textAlign = "center";
                ctx.textBaseline = "top";
                ctx.fillText(name, mapWidth/2, menuSize);
            });
        },
        update: function() {
            forEachCharBtn(function (btn) {
...
var preNewGameState = (function() {
    ...
    var menu = new Menu("",2*menuSize,0,mapWidth-4*menuSize,3*menuSize,menuSize,menuSize+"px ArcadeR", "#EEE");
...
var selectActState = (function() {
    ...
    var drawArrow = function(ctx,x,y,dir) {
        ...
        ctx.scale(1,dir);
        ctx.beginPath();
        ctx.moveTo(0,-menuSize/2);
        ctx.lineTo(menuSize,menuSize/2);
        ctx.lineTo(-menuSize,menuSize/2);
        ctx.closePath();
        ctx.fillStyle = "#FFF";
        ctx.fill();
    ...
    var buildMenu = function(act) {
        ...
        menu = new Menu("",2*menuSize,0,mapWidth-4*menuSize,3*menuSize,menuSize,menuSize+"px ArcadeR", "#EEE");
        var i;
        var range;
        menu.addSpacer(2);
        menu.addIconButton(
...
var aboutGameState = (function() {
    ...
    var drawDesc = function(ctx){
        ...
        for (i=0; i<numDescLines; i++) {
            ctx.fillText(desc[i],14*menuSize,y+i*2*menuSize);
        }
    };
...
var scoreState = (function(){
    ...
    var menu = new Menu("", 2*menuSize,mapHeight-6*menuSize,mapWidth-4*menuSize,3*menuSize,menuSize,menuSize+"px ArcadeR", "#EEE");
    menu.addTextButton("BACK",
        function() {
            exitTo(homeState);
        });
    ...
    var drawScoreBox = function(ctx) {
        ...
        x = 9*menuSize;
        y = 0;
        ctx.fillStyle = "#FFF"; ctx.fillText("HIGH SCORES", x+4*menuSize,y);
        y += menuSize*4;
        ...
        var drawContrails = function(x,y) {
            ...
            for (dy=-4; dy<=4; dy+=2) {
                ctx.beginPath();
                ctx.moveTo(x+menuSize,y+dy);
                ctx.lineTo(x+menuSize*(Math.random()*0.5+1.5),y+dy);
                ctx.stroke();
            }
            ctx.restore();
        ...
        atlas.drawPacmanSprite(ctx,x+2*menuSize,y+menuSize/2,DIR_LEFT,1);
        y += menuSize*2;
        ctx.fillStyle = scoreColor; ctx.fillText(highScores[1], x,y);
        drawContrails(x+2*menuSize,y+menuSize/2);
        atlas.drawPacmanSprite(ctx,x+2*menuSize,y+menuSize/2,DIR_LEFT,1);

        y += menuSize*3;
        ctx.fillStyle = scoreColor; ctx.fillText(highScores[2], x,y);
        atlas.drawMsPacmanSprite(ctx,x+2*menuSize,y+menuSize/2,DIR_LEFT,1);
        y += menuSize*2;
        ...
        atlas.drawOttoSprite(ctx,x+2*menuSize,y+menuSize/2,DIR_LEFT,0);
        y += menuSize*2;
        ...
        atlas.drawCookiemanSprite(ctx,x+2*menuSize,y+menuSize/2,DIR_LEFT,1);
        y += menuSize*2;
    ...
    var drawFood = function(ctx) {
        ctx.globalAlpha = 0.5;
        ctx.font = menuSize + "px sans-serif";
        ...
        ctx.fillStyle = "#FFF";
        ctx.fillText("10",x+menuSize,y);
        y += 1.5*menuSize;

        ctx.fillStyle = "#ffb8ae";
        ctx.beginPath();
        ctx.arc(x,y-0.5,menuSize/2,0,Math.PI*2);
        ctx.fill();
        ctx.fillStyle = "#FFF";
        ctx.fillText("50",x+menuSize,y);

        y += 3*menuSize;
        atlas.drawGhostSprite(ctx,x,y,0,DIR_RIGHT,true);
        atlas.drawGhostPoints(ctx,x+2*menuSize,y,200);
        ...
        atlas.drawGhostSprite(ctx,x+2*menuSize,y,0,DIR_RIGHT,true);
        atlas.drawGhostPoints(ctx,x+4*menuSize,y,400);
        ...
        atlas.drawGhostSprite(ctx,x+2*menuSize,y,0,DIR_RIGHT,true);
        ctx.globalAlpha = alpha;
        atlas.drawGhostSprite(ctx,x+4*menuSize,y,0,DIR_RIGHT,true);
        atlas.drawGhostPoints(ctx,x+6*menuSize,y,800);
        ...
        atlas.drawGhostSprite(ctx,x+4*menuSize,y,0,DIR_RIGHT,true);
        ctx.globalAlpha = alpha;
        atlas.drawGhostSprite(ctx,x+6*menuSize,y,0,DIR_RIGHT,true);
        atlas.drawGhostPoints(ctx,x+8*menuSize,y,1600);
        ...
        for (i=0; i<pac_fruits.length; i++) {
            f = pac_fruits[i];
            atlas.drawFruitSprite(ctx,x,y,f.name);
            atlas.drawPacFruitPoints(ctx,x+2*menuSize,y,f.points);
            y += 2*menuSize;
        }
        x += 6*menuSize;
        y = 13.5*menuSize;
        for (i=0; i<mspac_fruits.length; i++) {
            f = mspac_fruits[i];
            atlas.drawFruitSprite(ctx,x,y,f.name);
            atlas.drawMsPacFruitPoints(ctx,x+2*menuSize,y,f.points);
            y += 2*menuSize;
        }
        ctx.globalAlpha = 1;
    };
...
var aboutState = (function(){
    ...
    var menu = new Menu("", 2*menuSize,mapHeight-11*menuSize,mapWidth-4*menuSize,3*menuSize,menuSize,menuSize+"px ArcadeR", "#EEE");
...
var playState = {
    ...
    update: function() {
        invaders.update(executive.getFramePeriod());
    },
};
...
var overState = (function() {
    ...
    return {
        ...
        draw: function() {
            // renderer.blitMap();
            // renderer.drawScore();
            renderer.drawMessage("GAME OVER", "#F00", 13, 20);
        },
        update: function() {
            if (frames == 240) {
...
var scoreBoardState = (function() {
    ...
    var drawDesc = function(ctx){
        ...
        ctx.lineWidth = 2;
        ctx.beginPath();
        var x=0, y=9*menuSize, w=mapWidth, h=23*menuSize; // 2 * num_score_list + 3
        var r=2*menuSize/4;
        ctx.moveTo(x,y+r);
        ...
        ctx.textBaseline = "top";
        ctx.textAlign = "center";
        ctx.fillText("HI-SCORES",w/2,10*menuSize);
        ctx.fillStyle = "#EEE";
        var y = 12*menuSize;
        var i;
        ...
        if (num_score_list == 0) {
            ctx.textAlign = "center";
            ctx.fillText("NO SCORES FOUND",w/2,y+8*menuSize);
        }
    };
