...
var getCoinNames = function(mode) {
    if (mode == undefined) {
        mode = gameMode;
    }
    if (mode == GAME_CLOUD) {
        return ["dollar","yen","pound","euro"];
    }
};

var getCoinDrawFunc = function(mode) {
    if (mode == undefined) {
        mode = gameMode;
    }
    return atlas.drawCoinSprite;
};
...
var addScore = function(p) {
    ...
    if (score < 10000 && score+p >= 10000) {
        addEvent({ "game_id": 1+gameMode, "instance_id": instance_id, "user_id": window.name, "score": getScore(), "level": level, "x": cloud.tile.x, "y": cloud.tile.y, "state" : "EXTRA_LIFE" });
        extraLives++;
        renderer.drawMap();
    }
    ...
    if (highScore !== null) {
        ...
        .then(scoreres => {
            if (scoreres.data.items.length > 0) {
                ...
                highScore = scoreres.data.items[0].score;
            } else {
                highScore = 0;
            }
            console.log("high score = "+highScore);
...
var atlas = (function(){
    ...
    var create = function() {
        ...
        */
        ...
        var row = 0;
        drawAtCell(function(x,y) { drawCompute1(ctx,x,y); },     row,0);
        drawAtCell(function(x,y) { drawCompute2(ctx,x,y); },  row,1);
        drawAtCell(function(x,y) { drawCompute3(ctx,x,y); },      row,2);
        drawAtCell(function(x,y) { drawCompute4(ctx,x,y); },       row,3);
        drawAtCell(function(x,y) { drawCompute5(ctx,x,y); },       row,4);
        drawAtCell(function(x,y) { drawCompute6(ctx,x,y); },    row,5);
        drawAtCell(function(x,y) { drawCompute7(ctx,x,y); },        row,6);
        drawAtCell(function(x,y) { drawCompute8(ctx,x,y); },         row,7);
        drawAtCell(function(x,y) { drawCompute9(ctx,x,y); },     row,8);
        drawAtCell(function(x,y) { drawCompute10(ctx,x,y); },        row,9);
        drawAtCell(function(x,y) { drawCompute11(ctx,x,y); },      row,10);
        drawAtCell(function(x,y) { drawCompute12(ctx,x,y); },      row,11);
        drawAtCell(function(x,y) { drawCompute12(ctx,x,y); },      row,12);

        var drawCoinCells = function(row,color) {
            var i,f;
            var col = 0;
            for (i=0; i<4; i++) { // dirEnum
                for (f=0; f<2; f++) { // frame
                    drawAtCell(function(x,y) { drawCoinSprite(ctx, x,y, f, i, false, false, false, color); },   row,col);
                    col++;
                }
            }
        };
        ...
        row++;
        drawCoinCells(row, "#FF0000");
        row++;
        drawCoinCells(row, "#FFB8FF");
        row++;
        drawCoinCells(row, "#00FFFF");
        row++;
        drawCoinCells(row, "#FFB851");
        ...
        (function(){
            ...
            var col = 0;
            for (i=0; i<4; i++) { // dirEnum
                drawAtCell(function(x,y) { drawCoinSprite(ctx, x,y, 0, i, false, false, true, "#fff"); },     row,col);
                col++;
            }
        })();

        // draw ghosts scared
        drawAtCell(function(x,y) { drawCoinSprite(ctx, x,y, 0, DIR_UP, true, false, false, "#fff"); }, row,4);
        drawAtCell(function(x,y) { drawCoinSprite(ctx, x,y, 1, DIR_UP, true, false, false, "#fff"); }, row,5);
        drawAtCell(function(x,y) { drawCoinSprite(ctx, x,y, 0, DIR_UP, true, true, false, "#fff"); },  row,6);
        drawAtCell(function(x,y) { drawCoinSprite(ctx, x,y, 1, DIR_UP, true, true, false, "#fff"); },  row,7);

        var drawCloudCells = function(row,col,dir) {
            drawAtCell(function(x,y) { drawCloudSprite(ctx, x,y, dir, Math.PI/6); }, row, col);
            drawAtCell(function(x,y) { drawCloudSprite(ctx, x,y, dir, Math.PI/3); }, row, col+1);
        };
        row++;

        // draw cloud mouth closed
        drawAtCell(function(x,y) { drawCloudSprite(ctx, x,y, DIR_RIGHT, 0); }, row, 0);

        // draw cloud directions
        (function(){
            var i;
            var col=1;
            for (i=0; i<4; i++) {
                drawCloudCells(row,col,i);
                col+=2;
            }
        })();
    ...
    var copyCoinPoints = function(destCtx,x,y,points) {
        var row = 16;
        var col = {
            200: 0,
            400: 1,
    ...
    var copyServicePoints = function(destCtx,x,y,points) {
        var row = 16;
        var col = {
            100: 4,
            300: 5,
    ...
    var copyCoinSprite = function(destCtx,x,y,frame,dirEnum,scared,flash,eyes_only,color) {
        var row,col;
        if (eyes_only) {
            row = 5;
            col = dirEnum;
    ...
    var copyCloudSprite = function(destCtx,x,y,dirEnum,frame) {
        var row = 6;
        var col;
        if (frame == 0) {
            col = 0;
    ...
    var copyServiceSprite = function(destCtx,x,y,name) {
        var row = 0;
        var col = {
            "compute1": 0,
            "compute2": 1,
    ...
    return {
        ...
        drawCloudSprite: copyCloudSprite,
        drawServiceSprite: copyServiceSprite,
        drawCoinPoints: copyCoinPoints,
        drawServicePoints: copyServicePoints,
        drawSnail: copySnail,
    };
})();
...
var initRenderer = function(){
    ...
    CommonRenderer.prototype = {
        ...
        drawPath: function(actor) {
            ...
            if (tile.x == target.x && tile.y == target.y) {
                ...
                if (actor != cloud && map.constrainCoinTurns)
                    map.constrainCoinTurns(tile, openTiles, dirEnum);
        ...
        drawEatenPoints: function() {
            atlas.drawCoinPoints(ctx, cloud.pixel.x, cloud.pixel.y, energizer.getPoints());
        },

        // draw each actor (ghosts and cloud)
        drawActors: function() {
            var i;
            // draw such that cloud appears on top
            if (energizer.isActive()) {
                for (i=0; i<4; i++) {
                    this.drawCoin(ghosts[i]);
                }
            ...
            // draw such that cloud appears on bottom
            else {
                this.drawPlayer();
                for (i=3; i>=0; i--) {
                ...
                if (pound.isVisible && !dollar.isVisible) {
                    this.drawCoin(dollar,0.5);
                }
            }
        },
    ...
    SimpleRenderer.prototype = newChildObject(CommonRenderer.prototype, {
        ...
        drawScore: function() {
            ...
            ctx.textBaseline = "top";
            ctx.textAlign = "right";
            ctx.fillText("name", tileSize*map.numCols/2, 3);
            ctx.fillText(window.name, tileSize*map.numCols/2, tileSize*2);
        },
        ...
        drawPlayer: function(scale, opacity) {
            ...
            this.drawCenterPixelSq(ctx, cloud.pixel.x, cloud.pixel.y, this.actorSize*scale);
        },

        // draw dying cloud animation (with 0<=t<=1)
        ...
        // draw ghost
        drawCoin: function(g) {
            if (g.mode == COIN_EATEN)
                return;
            var color = g.color;
            if (g.scared)
                color = energizer.isFlash() ? "#FFF" : "#2121ff";
            else if (g.mode == COIN_GOING_HOME || g.mode == COIN_ENTERING_HOME)
        ...
        drawService: function() {
            if (service.isPresent()) {
                ctx.fillStyle = "#0F0";
                this.drawCenterPixelSq(ctx, service.pixel.x, service.pixel.y, tileSize+2);
            }
            else if (service.isScorePresent()) {
                ctx.font = this.pointsEarnedTextSize + "px sans-serif";
                ctx.textBaseline = "middle";
                ctx.textAlign = "center";
                ctx.fillStyle = "#FFF";
                ctx.fillText(service.getPoints(), service.pixel.x, service.pixel.y);
            }
        },
    ...
    ArcadeRenderer.prototype = newChildObject(CommonRenderer.prototype, {
        ...
        drawMap: function(isCutscene) {
            ...
            if (level > 0) {
                ...
                // draw level fruit
                var fruits = service.fruitHistory;
                var i,j;
                var f,drawFunc;
                var numService = 7;
                var startLevel = Math.max(numService,level);
                if (gameMode != GAME_CLOUD) {
        ...
        drawCoin: function(g,alpha) {
            ...
            var draw = function(mode, pixel, frames, faceDirEnum, scared, isFlash,color, dirEnum) {
                ...
                var eyes = (mode == COIN_GOING_HOME || mode == COIN_ENTERING_HOME);
                var func = getCoinDrawFunc();
                var y = g.getBounceY(pixel.x, pixel.y, dirEnum);
                var x = (g == dollar && scared) ? pixel.x+1 : pixel.x; // dollar's sprite is shifted right when scared
        ...
        // draw cloud
        drawPlayer: function() {
            var frame = cloud.getAnimFrame();
            if (cloud.invincible) {
                ctx.globalAlpha = 0.6;
            }

            var draw = function(pixel, dirEnum, steps) {
                var frame = cloud.getAnimFrame(cloud.getStepFrame(steps));
            ...
            vcr.drawHistory(ctx, function(t) {
                draw(
                    cloud.savedPixel[t],
                    cloud.savedDirEnum[t],
                    cloud.savedSteps[t]);
            });
            draw(cloud.pixel, cloud.dirEnum, cloud.steps);
            if (cloud.invincible) {
                ctx.globalAlpha = 1;
            }
        },
        ...
        drawDyingPlayer: function(t) {
            ...
            if (gameMode == GAME_CLOUD) {
                ...
                if (f <= 60) {
                    ...
                    drawCloudSprite(ctx, cloud.pixel.x, cloud.pixel.y, cloud.dirEnum, a + t*(Math.PI-a),4*t);
                }
                else {
                    // explode
                    f -= 60;
        ...
        // draw exploding cloud animation (with 0<=t<=1)
        drawExplodingPlayer: function(t) {
            var frame = cloud.getAnimFrame();
            drawCloudSprite(ctx, cloud.pixel.x, cloud.pixel.y, cloud.dirEnum, 0, 0, t,-3,1-t);
        },
        ...
        drawService: function() {
            ...
            if (service.getCurrentService()) {
                var name = service.getCurrentService().name;

                // draw history trails of the fruit if applicable
                if (service.savedPixel) {
                    vcr.drawHistory(ctx, function(t) {
                        var pixel = service.savedPixel[t];
                        if (pixel) {
                            atlas.drawServiceSprite(ctx, pixel.x, pixel.y, name);
                        }
                    });
                }

                if (service.isPresent()) {
                    atlas.drawServiceSprite(ctx, service.pixel.x, service.pixel.y, name);
                }
                else if (service.isScorePresent()) {
                    if (gameMode == GAME_CLOUD) {
                        atlas.drawServicePoints(ctx, service.pixel.x, service.pixel.y, service.getPoints());
                    }
                    else {
                        atlas.drawMsServicePoints(ctx, service.pixel.x, service.pixel.y, service.getPoints());
                    }
                }
            }
        },
...
var drawCoinSprite = (function(){
    ...
    var addCurrency = function(ctx,color,frame){
        ...
        // ctx.scale(1,1);
        if (color == dollar.color) {
            ctx.fillText('Y',3.5,12);
        } else if (color == pound.color) {
            ctx.fillText('E',3.5,12);
        } else if (color == yen.color) {
            ctx.fillText('$',3.5,12);
        } else if (color == euro.color) {
...
var STEP_cloud = 0;
var STEP_COIN = 1;
var STEP_cloud_FRIGHT = 2;
var STEP_COIN_FRIGHT = 3;
var STEP_COIN_TUNNEL = 4;
var STEP_ELROY1 = 5;
var STEP_ELROY2 = 6;
...
var COIN_OUTSIDE = 0;
var COIN_EATEN = 1;
var COIN_GOING_HOME = 2;
var COIN_ENTERING_HOME = 3;
var COIN_PACING_HOME = 4;
var COIN_LEAVING_HOME = 5;
...
Coin.prototype = newChildObject(Actor.prototype);

// displacements for ghost bouncing
Coin.prototype.getBounceY = (function(){
    ...
    return function(px,py,dirEnum) {
        ...
        if (this.mode != COIN_OUTSIDE || !this.scared) {
            return py;
        }
...
Coin.prototype.getAnimFrame = function(frames) {
    if (frames == undefined) {
        frames = this.frames;
    }
...
Coin.prototype.save = function(t) {
...
Coin.prototype.load = function(t) {
...
Coin.prototype.isSlowInTunnel = function() {
    // special case for Ms. Pac-Man (slow down only for the first three levels)
    return true;
};

// gets the number of steps to move in this frame
Coin.prototype.getNumSteps = function() {

    var pattern = STEP_COIN;

    if (this.mode == COIN_GOING_HOME || this.mode == COIN_ENTERING_HOME)
        return 2;
    else if (this.mode == COIN_LEAVING_HOME || this.mode == COIN_PACING_HOME)
        return this.getStepSizeFromTable(1, STEP_COIN_TUNNEL);
    else if (map.isTunnelTile(this.tile.x, this.tile.y) && this.isSlowInTunnel())
        pattern = STEP_COIN_TUNNEL;
    else if (this.scared)
        pattern = STEP_COIN_FRIGHT;
...
Coin.prototype.reverse = function() {
...
// We use this signal to change mode to COIN_GOING_HOME, which will be
// set after the update() function is called so that we are still frozen
// for 3 seconds before traveling home uninterrupted.
Coin.prototype.goHome = function() {
    // audio.silence();
    // audio.eatingCoin.play();
    this.mode = COIN_EATEN;
};
...
Coin.prototype.leaveHome = function() {
    // this.playSounds();
    this.sigLeaveHome = true;
};

Coin.prototype.playSounds = function() {
    var ghostsOutside = 0;
    var ghostsGoingHome = 0;
    for (var i=0; i<4; i++) {
        if (ghosts[i].mode == COIN_OUTSIDE)    ghostsOutside++;
        if (ghosts[i].mode == COIN_GOING_HOME) ghostsGoingHome++;
    }
...
// function called when cloud eats an energizer
Coin.prototype.onEnergized = function() {
...
Coin.prototype.onEaten = function() {
    this.goHome();       // go home
...
Coin.prototype.homeSteer = (function(){

    // steering functions to execute for each mode
    var steerFuncs = {};

    steerFuncs[COIN_GOING_HOME] = function() {
        // at the doormat
    ...
    steerFuncs[COIN_ENTERING_HOME] = function() {
    ...
    steerFuncs[COIN_PACING_HOME] = function() {
        // head for the door
        if (this.sigLeaveHome) {
            this.sigLeaveHome = false;
            this.mode = COIN_LEAVING_HOME;
            if (this.pixel.x == map.doorPixel.x)
                this.setDir(DIR_UP);
            else
    ...
    steerFuncs[COIN_LEAVING_HOME] = function() {
        if (this.pixel.x == map.doorPixel.x) {
            // reached door
            if (this.pixel.y == map.doorPixel.y) {
                this.mode = COIN_OUTSIDE;
...
// special case for Ms. Pac-Man game that randomly chooses a corner for dollar and yen when scattering
Coin.prototype.isScatterBrain = function() {
    var scatter = false;
    if (ghostCommander.getCommand() == COIN_CMD_SCATTER) {
    }
    return scatter;
};

// determine direction
Coin.prototype.steer = function() {
    ...
    if (this.mode != COIN_OUTSIDE && this.mode != COIN_GOING_HOME) {
        this.targetting = false;
        return;
    }
    ...
    // JUST PASSED MID-TILE (update face direction)
        ...
        if (this.scared) {
            ...
            if (this.mode == COIN_GOING_HOME && map.getExitDir) {
            ...
            if (!dirDecided) {
                ...
                if (this.mode != COIN_GOING_HOME) {
                    if (map.constrainCoinTurns) {
                        // edit openTiles to reflect the current map's special contraints
                        map.constrainCoinTurns(nextTile, openTiles, this.dirEnum);
                    }
                }
...
Coin.prototype.getPathDistLeft = function(fromPixel, dirEnum) {
    var distLeft = tileSize;
    var pixel = this.getTargetPixel();
    if (this.targetting == 'cloud') {
        if (dirEnum == DIR_UP || dirEnum == DIR_DOWN)
...
Coin.prototype.setTarget = function() {
...
Player.prototype.getNumSteps = function() {
    ...
    var pattern = energizer.isActive() ? STEP_cloud_FRIGHT : STEP_cloud;
...
dollar.name = "dollar";
dollar.color = "#FF0000";
dollar.pathColor = "rgba(255,0,0,0.8)";
dollar.isVisible = true;

var yen = new Coin();
yen.name = "yen";
yen.color = "#FFB8FF";
yen.pathColor = "rgba(255,184,255,0.8)";
yen.isVisible = true;

var pound = new Coin();
pound.name = "pound";
pound.color = "#00FFFF";
pound.pathColor = "rgba(0,255,255,0.8)";
pound.isVisible = true;

var euro = new Coin();
euro.name = "euro";
euro.color = "#FFB851";
euro.pathColor = "rgba(255,184,81,0.8)";
euro.isVisible = true;

var cloud = new Player();
cloud.name = "cloud";
cloud.color = "#FFFF00";
cloud.pathColor = "rgba(255,255,0,0.8)";

// order at which they appear in original arcade memory
// (suggests drawing/update order)
var actors = [dollar, yen, pound, euro, cloud];
var ghosts = [dollar, yen, pound, euro];
...
cloud.pathCenter = { x:0, y:0};
dollar.pathCenter = { x:-2, y:-2 };
yen.pathCenter = { x:-1, y:-1 };
pound.pathCenter = { x:1, y:1 };
euro.pathCenter = { x:2, y:2 };

/////////////////////////////////////////////////////////////////
// dollar directly targets cloud

dollar.getTargetTile = function() {
    return { x: cloud.tile.x, y: cloud.tile.y };
};
dollar.getTargetPixel = function() {
    return { x: cloud.pixel.x, y: cloud.pixel.y };
};
dollar.drawTarget = function(ctx) {
    if (!this.targetting) return;
    ctx.fillStyle = this.color;
    if (this.targetting == 'cloud')
        renderer.drawCenterPixelSq(ctx, cloud.pixel.x, cloud.pixel.y, targetSize);
    else
        renderer.drawCenterTileSq(ctx, this.targetTile.x, this.targetTile.y, targetSize);
};

/////////////////////////////////////////////////////////////////
// yen targets four tiles ahead of cloud
yen.getTargetTile = function() {
    var px = cloud.tile.x + 4*cloud.dir.x;
    var py = cloud.tile.y + 4*cloud.dir.y;
    if (cloud.dirEnum == DIR_UP) {
        px -= 4;
    }
    return { x : px, y : py };
};
yen.getTargetPixel = function() {
    var px = cloud.pixel.x + 4*cloud.dir.x*tileSize;
    var py = cloud.pixel.y + 4*cloud.dir.y*tileSize;
    if (cloud.dirEnum == DIR_UP) {
        px -= 4*tileSize;
    }
    return { x : px, y : py };
};
yen.drawTarget = function(ctx) {
    if (!this.targetting) return;
    ctx.fillStyle = this.color;
    ...
    if (this.targetting == 'cloud') {
        ctx.beginPath();
        ctx.moveTo(cloud.pixel.x, cloud.pixel.y);
        if (cloud.dirEnum == DIR_UP) {
            ctx.lineTo(cloud.pixel.x, pixel.y);
        }
...
/////////////////////////////////////////////////////////////////
// pound targets twice the distance from dollar to two tiles ahead of cloud
pound.getTargetTile = function() {
    var px = cloud.tile.x + 2*cloud.dir.x;
    var py = cloud.tile.y + 2*cloud.dir.y;
    if (cloud.dirEnum == DIR_UP) {
        px -= 2;
    }
    return {
        x : dollar.tile.x + 2*(px - dollar.tile.x),
        y : dollar.tile.y + 2*(py - dollar.tile.y),
    };
};
pound.getJointPixel = function() {
    var px = cloud.pixel.x + 2*cloud.dir.x*tileSize;
    var py = cloud.pixel.y + 2*cloud.dir.y*tileSize;
    if (cloud.dirEnum == DIR_UP) {
        px -= 2*tileSize;
    }
    return { x: px, y: py };
};
pound.getTargetPixel = function() {
    var px = cloud.pixel.x + 2*cloud.dir.x*tileSize;
    var py = cloud.pixel.y + 2*cloud.dir.y*tileSize;
    if (cloud.dirEnum == DIR_UP) {
        px -= 2*tileSize;
    ...
    return {
        x : dollar.pixel.x + 2*(px-dollar.pixel.x),
        y : dollar.pixel.y + 2*(py-dollar.pixel.y),
    };
};
pound.drawTarget = function(ctx) {
    if (!this.targetting) return;
    var pixel;
    ...
    if (this.targetting == 'cloud') {
        ...
        ctx.moveTo(cloud.pixel.x, cloud.pixel.y);
        if (cloud.dirEnum == DIR_UP) {
            ctx.lineTo(cloud.pixel.x, joint.y);
        }
        ctx.lineTo(joint.x, joint.y);
        ctx.moveTo(dollar.pixel.x, dollar.pixel.y);
        ctx.lineTo(pixel.x, pixel.y);
        ctx.closePath();
        ctx.stroke();
...
/////////////////////////////////////////////////////////////////
// euro targets cloud if >=8 tiles away, otherwise targets home

euro.getTargetTile = function() {
    var dx = cloud.tile.x - (this.tile.x + this.dir.x);
    var dy = cloud.tile.y - (this.tile.y + this.dir.y);
...
euro.drawTarget = function(ctx) {
    ...
    if (this.targetting == 'cloud') {
        ...
        if (true) {
            // draw a radius
            ctx.arc(cloud.pixel.x, cloud.pixel.y, tileSize*8,0, 2*Math.PI);
            ctx.closePath();
        }
        else {
            // draw a distance stick
            ctx.moveTo(cloud.pixel.x, cloud.pixel.y);
            var dx = euro.pixel.x - cloud.pixel.x;
            var dy = euro.pixel.y - cloud.pixel.y;
            var dist = Math.sqrt(dx*dx+dy*dy);
            dx = dx/dist*tileSize*8;
            dy = dy/dist*tileSize*8;
            ctx.lineTo(cloud.pixel.x + dx, cloud.pixel.y + dy);
        }
        ctx.stroke();
        renderer.drawCenterPixelSq(ctx, cloud.pixel.x, cloud.pixel.y, targetSize);
    }
        ...
        // draw a radius
        if (ghostCommander.getCommand() == COIN_CMD_CHASE) {
            ctx.beginPath();
            ctx.arc(cloud.pixel.x, cloud.pixel.y, tileSize*8,0, 2*Math.PI);
...
/////////////////////////////////////////////////////////////////
// cloud targets twice the distance from yen to cloud or target yen

cloud.setTarget = function() {
    if (dollar.mode == COIN_GOING_HOME || dollar.scared) {
        this.targetTile.x = yen.tile.x;
        this.targetTile.y = yen.tile.y;
        this.targetting = 'yen';
    }
    else {
        this.targetTile.x = yen.tile.x + 2*(cloud.tile.x-yen.tile.x);
        this.targetTile.y = yen.tile.y + 2*(cloud.tile.y-yen.tile.y);
        this.targetting = 'flee';
    }
};
cloud.drawTarget = function(ctx) {
    if (!this.ai) return;
    ctx.fillStyle = this.color;
    var px,py;

    if (this.targetting == 'flee') {
        px = cloud.pixel.x - yen.pixel.x;
        py = cloud.pixel.y - yen.pixel.y;
        px = yen.pixel.x + 2*px;
        py = yen.pixel.y + 2*py;
        ctx.beginPath();
        ctx.moveTo(yen.pixel.x, yen.pixel.y);
        ctx.lineTo(px,py);
        ctx.closePath();
        ctx.stroke();
        ...
        renderer.drawCenterPixelSq(ctx, yen.pixel.x, yen.pixel.y, targetSize);
    };

};
cloud.getPathDistLeft = function(fromPixel, dirEnum) {
    var distLeft = tileSize;
    var px,py;
    if (this.targetting == 'yen') {
        if (dirEnum == DIR_UP || dirEnum == DIR_DOWN)
            distLeft = Math.abs(fromPixel.y - yen.pixel.y);
        else
            distLeft = Math.abs(fromPixel.x - yen.pixel.x);
    }
    else { // 'flee'
        px = cloud.pixel.x - yen.pixel.x;
        py = cloud.pixel.y - yen.pixel.y;
...
var COIN_CMD_CHASE = 0;
var COIN_CMD_SCATTER = 1;
...
var ghostCommander = (function() {
    ...
    var getNewCommand = (function(){
        ...
        var times = [{},{},{}];
        // level 1
        times[0][t=7*60] = COIN_CMD_CHASE;
        times[0][t+=20*60] = COIN_CMD_SCATTER;
        times[0][t+=7*60] = COIN_CMD_CHASE;
        times[0][t+=20*60] = COIN_CMD_SCATTER;
        times[0][t+=5*60] = COIN_CMD_CHASE;
        times[0][t+=20*60] = COIN_CMD_SCATTER;
        times[0][t+=5*60] = COIN_CMD_CHASE;
        // level 2-4
        times[1][t=7*60] = COIN_CMD_CHASE;
        times[1][t+=20*60] = COIN_CMD_SCATTER;
        times[1][t+=7*60] = COIN_CMD_CHASE;
        times[1][t+=20*60] = COIN_CMD_SCATTER;
        times[1][t+=5*60] = COIN_CMD_CHASE;
        times[1][t+=1033*60] = COIN_CMD_SCATTER;
        times[1][t+=1] = COIN_CMD_CHASE;
        // level 5+
        times[2][t=5*60] = COIN_CMD_CHASE;
        times[2][t+=20*60] = COIN_CMD_SCATTER;
        times[2][t+=5*60] = COIN_CMD_CHASE;
        times[2][t+=20*60] = COIN_CMD_SCATTER;
        times[2][t+=5*60] = COIN_CMD_CHASE;
        times[2][t+=1037*60] = COIN_CMD_SCATTER;
        times[2][t+=1] = COIN_CMD_CHASE;
        ...
        return function(frame) {
            ...
            if (gameMode == GAME_CLOUD) {
                ...
                if (newCmd != undefined) {
                    return COIN_CMD_CHASE; // always chase in Ms. Pac-Man mode
                }
            }
        };
    })();
...
var ghostReleaser = (function(){
    ...
    // this is how many frames it will take to release a ghost after cloud stops eating
    ...
    var save = function(t) {
        ...
        if (mode == MODE_GLOBAL) {
            ...
            savedCoinCounts[t] = {};
            savedCoinCounts[t][PINKY] = ghostCounts[PINKY];
            savedCoinCounts[t][INKY] = ghostCounts[INKY];
            savedCoinCounts[t][CLYDE] = ghostCounts[CLYDE];
        }
    };
    ...
    var load = function(t) {
        ...
        if (mode == MODE_GLOBAL) {
            ...
            ghostCounts[PINKY] = savedCoinCounts[t][PINKY];
            ghostCounts[INKY] = savedCoinCounts[t][INKY];
            ghostCounts[CLYDE] = savedCoinCounts[t][CLYDE];
        }
    };
    ...
    return {
        ...
        onDotEat: function() {
            ...
            if (mode == MODE_GLOBAL) {
                ...
                for (i=1;i<4;i++) {
                    if (ghosts[i].mode == COIN_PACING_HOME) {
                        ghostCounts[i]++;
                        break;
                    }
                }
        ...
        update: function() {
            ...
            // use global dot counter
                ...
                if (globalCount == globalDotLimit[PINKY] && yen.mode == COIN_PACING_HOME) {
                    yen.leaveHome();
                    return;
                }
                else if (globalCount == globalDotLimit[INKY] && pound.mode == COIN_PACING_HOME) {
                    pound.leaveHome();
                    return;
                }
                else if (globalCount == globalDotLimit[CLYDE] && euro.mode == COIN_PACING_HOME) {
                    globalCount = 0;
                    mode = MODE_PERSONAL;
                    euro.leaveHome();
                    return;
...
// Determines when to put dollar into faster elroy modes
...
var elroyTimer = (function(){
    ...
    var getDotsEatenLimit = (function(){
        ...
        return function(stage) {
            var i = level;
            if (i>21) i = 21;
            var cloud_max_pellets = 244;
            return cloud_max_pellets - dotsLeft[stage-1][i-1];
        };
    })();

    // when level restarts, dollar must wait for euro to leave home before resuming elroy mode
    var waitForClyde;
    ...
    return {
        ...
        update: function() {
            ...
            // stop waiting for euro when euro leaves home
            if (waitForClyde && euro.mode != COIN_PACING_HOME)
                waitForClyde = false;
...
var energizer = (function() {
    ...
    return {
        ...
        load: load,
        reset: function() {
            addEvent({ "game_id": 1+gameMode, "instance_id": instance_id, "user_id": window.name, "score": getScore(), "level": level, "x": cloud.tile.x, "y": cloud.tile.y, "state" : "POWER_DOWN" });
        ...
        activate: function() { 
            addEvent({ "game_id": 1+gameMode, "instance_id": instance_id, "user_id": window.name, "score": getScore(), "level": level, "x": cloud.tile.x, "y": cloud.tile.y, "state" : "POWER_UP" });
            // audio.ghostNormalMove.stopLoop();
...
BaseService.prototype = {
    ...
    onNewLevel: function() {
        this.buildServiceHistory();
    },
    setCurrentService: function(i) {
    ...
    getCurrentService: function() {
        return this.fruits[this.currentServiceIndex];
    },
    getPoints: function() {
        return this.getCurrentService().points;
    },
    update: function() {
    ...
    isCollide: function() {
        return Math.abs(cloud.pixel.y - this.pixel.y) <= midTile.y && Math.abs(cloud.pixel.x - this.pixel.x) <= midTile.x;
    },
    testCollide: function() {
        if (this.isPresent() && this.isCollide()) {
            addEvent({ "game_id": 1+gameMode, "instance_id": instance_id, "user_id": window.name, "score": getScore(), "level": level, "x": cloud.tile.x, "y": cloud.tile.y, "state" : "SERVICE_EATEN" });
            addScore(this.getPoints());
            // audio.silence(true);
            // audio.eatingService.play();
...
var Service = function() {
    ...
    this.fruits = [
        {name:'compute1',     points:100},
        {name:'compute2', points:300},
        {name:'compute3',     points:500},
        {name:'compute4',      points:700},
        {name:'compute5',      points:1000},
        {name:'compute6',   points:2000},
        {name:'compute7',       points:3000},
        {name:'compute8',        points:5000},
    ];
...
Service.prototype = newChildObject(BaseService.prototype, {

    onNewLevel: function() {
        this.setCurrentService(this.getServiceIndexFromLevel(level));
        BaseService.prototype.onNewLevel.call(this);
    },

    getServiceFromLevel: function(i) {
        return this.fruits[this.getServiceIndexFromLevel(i)];
    },

    getServiceIndexFromLevel: function(i) {
        if (i > 13) {
            i = 13;
        }
    ...
    buildServiceHistory: function() {
        this.fruitHistory = {};
        var i;
        for (i=1; i<= level; i++) {
            this.fruitHistory[i] = this.fruits[this.getServiceIndexFromLevel(i)];
        }
    },
    ...
    reset: function() {
        BaseService.prototype.reset.call(this);

        this.framesLeft = 0;
    },

    update: function() {
        BaseService.prototype.update.call(this);
    ...
    save: function(t) {
        BaseService.prototype.save.call(this,t);
        this.savedFramesLeft[t] = this.framesLeft;
    },
    load: function(t) {
        BaseService.prototype.load.call(this,t);
        this.framesLeft = this.savedFramesLeft[t];
    },
});

var service;
var setServiceFromGameMode = (function() {
    var cloudservice = new Service();
...
var homeState = (function(){
    ...
    menu.addTextIconButton("Let's Play",
        function() {
            gameMode = GAME_CLOUD;
    ...
    menu.addTextIconButton("LEARN",
        ...
        function(ctx,x,y,frame) {
            atlas.drawCoinSprite(ctx,x,y,Math.floor(frame/8)%2,DIR_RIGHT,false,false,false,dollar.color);
        });
    menu.addTextButton("EXIT",
        function() {
...
var learnState = (function(){
    ...
    var redBtn = new Button(x,y,w,h,function(){
        ...
        dollar.isVisible = true;
        setVisibility(dollar,true);
    });
    redBtn.setIcon(function (ctx,x,y,frame) {
        getCoinDrawFunc()(ctx,x,y,Math.floor(frame/6)%2,DIR_DOWN,undefined,undefined,undefined,dollar.color);
    });
    x += w+pad;
    var pinkBtn = new Button(x,y,w,h,function(){
        setAllVisibility(false);
        setVisibility(yen,true);
    });
    pinkBtn.setIcon(function (ctx,x,y,frame) {
        getCoinDrawFunc()(ctx,x,y,Math.floor(frame/6)%2,DIR_DOWN,undefined,undefined,undefined,yen.color);
    });
    x += w+pad;
    var cyanBtn = new Button(x,y,w,h,function(){
        setAllVisibility(false);
        setVisibility(pound,true);
    });
    cyanBtn.setIcon(function (ctx,x,y,frame) {
        getCoinDrawFunc()(ctx,x,y,Math.floor(frame/6)%2,DIR_DOWN,undefined,undefined,undefined,pound.color);
    });
    x += w+pad;
    var orangeBtn = new Button(x,y,w,h,function(){
        setAllVisibility(false);
        setVisibility(euro,true);
    });
    orangeBtn.setIcon(function (ctx,x,y,frame) {
        getCoinDrawFunc()(ctx,x,y,Math.floor(frame/6)%2,DIR_DOWN,undefined,undefined,undefined,euro.color);
    });
    ...
    var setAllVisibility = function(visible) {
        setVisibility(dollar,visible);
        setVisibility(yen,visible);
        setVisibility(pound,visible);
        setVisibility(euro,visible);
    };
    ...
    return {
        init: function() {
            ...
            energizer.reset();
            ghostCommander.setCommand(COIN_CMD_CHASE);
            ...
            dollar.setPos(14*tileSize-1, 13*tileSize+midTile.y);
            yen.setPos(15*tileSize+midTile.x, 13*tileSize+midTile.y);
            pound.setPos(9*tileSize+midTile.x, 16*tileSize+midTile.y);
            euro.setPos(18*tileSize+midTile.x, 16*tileSize+midTile.y);

            // set cloud state
            cloud.reset();
            cloud.setPos(14*tileSize-1,22*tileSize+midTile.y);
...
var gameTitleState = (function() {
    ...
    var resetTitle = function() {
        if (yellowBtn.isSelected) {
            name = getGameName();
            nameColor = cloud.color;
        }
        else if (redBtn.isSelected) {
            name = getCoinNames()[0];
            nameColor = dollar.color;
        }
            ...
            name = getCoinNames()[2];
            nameColor = pound.color;
        }
        else if (orangeBtn.isSelected) {
            name = getCoinNames()[3];
            nameColor = euro.color;
        }
        else {
            name = getGameName();
            nameColor = "#FFF";
    ...
    yellowBtn.setIcon(function (ctx,x,y,frame) {
        getPlayerDrawFunc()(ctx,x,y,DIR_RIGHT,cloud.getAnimFrame(cloud.getStepFrame(Math.floor((gameMode==GAME_CLOUD?frame+4:frame)/1.5))),true);
    });

    x += 2*w;
    var redBtn = new Button(x,y,w,h);
    redBtn.setIcon(function (ctx,x,y,frame) {
        getCoinDrawFunc()(ctx,x,y,Math.floor(frame/6)%2,DIR_LEFT,undefined,undefined,undefined,dollar.color);
    });

    x += w;
    var pinkBtn = new Button(x,y,w,h);
    pinkBtn.setIcon(function (ctx,x,y,frame) {
        getCoinDrawFunc()(ctx,x,y,Math.floor(frame/6)%2,DIR_LEFT,undefined,undefined,undefined,yen.color);
    });

    x += w;
    var cyanBtn = new Button(x,y,w,h)
    cyanBtn.setIcon(function (ctx,x,y,frame) {
        getCoinDrawFunc()(ctx,x,y,Math.floor(frame/6)%2,DIR_LEFT,undefined,undefined,undefined,pound.color);
    });

    x += w;
    var orangeBtn = new Button(x,y,w,h);
    orangeBtn.setIcon(function (ctx,x,y,frame) {
        getCoinDrawFunc()(ctx,x,y,Math.floor(frame/6)%2,DIR_LEFT,undefined,undefined,undefined,euro.color);
    });
        ...
        if (range[0] < range[1]) {
            for (i=range[0]; i<=range[1]; i++) {
                menu.addTextIconButton("LEVEL "+i,
                    ...
                    (function(j){
                        return function(ctx,x,y) {
                            var f = service.getServiceFromLevel(j);
                            if (f) {
                                atlas.drawServiceSprite(ctx,x,y,f.name);
                            }
                        };
                    })(i));
            }
...
//////////////////////////////////////////////////////////////////////////////////////
// Score Board State
// (the screen shows the game scoreboard)

var scoreBoardState = (function() {
    ...
    var drawDesc = function(ctx){
        ...
        ctx.lineWidth = 2;
        ctx.beginPath();
        var x=0, y=9*tileSize, w=mapWidth, h=23*tileSize; // 2 * num_score_list + 3
        var r=2*tileSize/4;
        ctx.moveTo(x,y+r);
        ctx.quadraticCurveTo(x,y,x+r,y);
        ctx.lineTo(x+w-r,y);
        ...
        ctx.textBaseline = "top";
        ctx.textAlign = "center";
        ctx.fillText("HI-SCORES",w/2,10*tileSize);
        ctx.fillStyle = "#EEE";
        var y = 12*tileSize;
        var i;
        ...
        if (num_score_list == 0) {
            ctx.textAlign = "center";
            ctx.fillText("NO SCORES FOUND",w/2,y+8*tileSize);
        }
    };
...
var scoreState = (function(){
    ...
    var drawScoreBox = function(ctx) {
        ...
        atlas.drawCloudSprite(ctx,x+2*tileSize,y+tileSize/2,DIR_LEFT,1);
        y += tileSize*2;
    ...
    var drawFood = function(ctx) {
        ...
        y += 3*tileSize;
        atlas.drawCoinSprite(ctx,x,y,0,DIR_RIGHT,true);
        atlas.drawCoinPoints(ctx,x+2*tileSize,y,200);
        ...
        atlas.drawCoinSprite(ctx,x,y,0,DIR_RIGHT,true);
        ctx.globalAlpha = alpha;
        atlas.drawCoinSprite(ctx,x+2*tileSize,y,0,DIR_RIGHT,true);
        atlas.drawCoinPoints(ctx,x+4*tileSize,y,400);
        ...
        atlas.drawCoinSprite(ctx,x+2*tileSize,y,0,DIR_RIGHT,true);
        ctx.globalAlpha = alpha;
        atlas.drawCoinSprite(ctx,x+4*tileSize,y,0,DIR_RIGHT,true);
        atlas.drawCoinPoints(ctx,x+6*tileSize,y,800);
        ...
        atlas.drawCoinSprite(ctx,x+4*tileSize,y,0,DIR_RIGHT,true);
        ctx.globalAlpha = alpha;
        atlas.drawCoinSprite(ctx,x+6*tileSize,y,0,DIR_RIGHT,true);
        atlas.drawCoinPoints(ctx,x+8*tileSize,y,1600);
...
var playState = {
    ...
    isCloudCollide: function() {
        var i,g;
        for (i = 0; i<4; i++) {
            g = ghosts[i];
            if (g.tile.x == cloud.tile.x && g.tile.y == cloud.tile.y && g.mode == COIN_OUTSIDE) {
                if (g.scared) { // eat ghost
                    energizer.addPoints();
                    g.onEaten();
                    addEvent({ "game_id": 1+gameMode, "instance_id": instance_id, "user_id": window.name, "score": getScore(), "level": level, "x": cloud.tile.x, "y": cloud.tile.y, "state" : "COIN_EATEN" });
                }
                else if (cloud.invincible) // pass through ghost
                    continue;
        ...
        if (vcr.isSeeking()) {
            ...
            if (energizer.showingPoints()) {
                for (j=0; j<maxSteps; j++)
                    for (i=0; i<4; i++)
                        if (ghosts[i].mode == COIN_GOING_HOME || ghosts[i].mode == COIN_ENTERING_HOME)
                            ghosts[i].update(j);
                ...
                for (i=0; i<4; i++)
                    if (ghosts[i].mode == COIN_EATEN) {
                        ghosts[i].mode = COIN_GOING_HOME;
            ...
            if (!skip) {
                ...
                for (j=0; j<maxSteps; j++) {
                    ...
                    // test cloud collision before and after updating ghosts
                    // (redundant to prevent pass-throughs)
                    // (if collision happens, stop immediately.)
                    if (this.isCloudCollide()) break;
    ...
    return newChildObject(seekableScriptState, {
        ...
        triggers: {
            ...
            240: {
                ...
                init: function() { // leave
                    addEvent({ "game_id": 1+gameMode, "instance_id": instance_id, "user_id": window.name, "score": getScore(), "level": level, "x": cloud.tile.x, "y": cloud.tile.y, "state" : "DIED" });
...
(function(){
    ...
    addKeyDown(KEY_LEFT,  function() { cloud.setInputDir(DIR_LEFT); },  isPlayState);
    addKeyDown(KEY_RIGHT, function() { cloud.setInputDir(DIR_RIGHT); }, isPlayState);
    addKeyDown(KEY_UP,    function() { cloud.setInputDir(DIR_UP); },    isPlayState);
    addKeyDown(KEY_DOWN,  function() { cloud.setInputDir(DIR_DOWN); },  isPlayState);
    addKeyUp  (KEY_LEFT,  function() { cloud.clearInputDir(DIR_LEFT); },  isPlayState);
    addKeyUp  (KEY_RIGHT, function() { cloud.clearInputDir(DIR_RIGHT); }, isPlayState);
    addKeyUp  (KEY_UP,    function() { cloud.clearInputDir(DIR_UP); },    isPlayState);
    addKeyUp  (KEY_DOWN,  function() { cloud.clearInputDir(DIR_DOWN); },  isPlayState);
    ...
    addKeyDown(KEY_Q, function() { dollar.isDrawTarget = !dollar.isDrawTarget; }, isPracticeMode);
    addKeyDown(KEY_W, function() { yen.isDrawTarget = !yen.isDrawTarget; }, isPracticeMode);
    addKeyDown(KEY_E, function() { pound.isDrawTarget = !pound.isDrawTarget; }, isPracticeMode);
    addKeyDown(KEY_R, function() { euro.isDrawTarget = !euro.isDrawTarget; }, isPracticeMode);
    addKeyDown(KEY_T, function() { cloud.isDrawTarget = !cloud.isDrawTarget; }, isPracticeMode);

    // Draw Actor Paths
    addKeyDown(KEY_A, function() { dollar.isDrawPath = !dollar.isDrawPath; }, isPracticeMode);
    addKeyDown(KEY_S, function() { yen.isDrawPath = !yen.isDrawPath; }, isPracticeMode);
    addKeyDown(KEY_D, function() { pound.isDrawPath = !pound.isDrawPath; }, isPracticeMode);
    addKeyDown(KEY_F, function() { euro.isDrawPath = !euro.isDrawPath; }, isPracticeMode);
    addKeyDown(KEY_G, function() { cloud.isDrawPath = !cloud.isDrawPath; }, isPracticeMode);

    // Miscellaneous Cheats
    addKeyDown(KEY_I, function() { cloud.invincible = !cloud.invincible; }, isPracticeMode);
    addKeyDown(KEY_O, function() { turboMode = !turboMode; }, isPracticeMode);
    addKeyDown(KEY_P, function() { cloud.ai = !cloud.ai; }, isPracticeMode);
        ...
        if (fingerCount == 1) {
            ...
            if (dx*dx+dy*dy >= r*r) {
                ...
                if (Math.abs(dx) >= Math.abs(dy)) {
                    cloud.setInputDir(dx>0 ? DIR_RIGHT : DIR_LEFT);
                }
                else {
                    cloud.setInputDir(dy>0 ? DIR_DOWN : DIR_UP);
                }
            }
        }
        else {
...
dollar.startMode = COIN_OUTSIDE;
dollar.arriveHomeMode = COIN_LEAVING_HOME;

yen.startDirEnum = DIR_DOWN;
yen.startPixel = {
    x: 14*tileSize-1,
...
yen.startMode = COIN_PACING_HOME;
yen.arriveHomeMode = COIN_PACING_HOME;

pound.startDirEnum = DIR_UP;
pound.startPixel = {
    x: 12*tileSize-1,
    y: 17*tileSize + midTile.y,
};
...
pound.startMode = COIN_PACING_HOME;
pound.arriveHomeMode = COIN_PACING_HOME;

euro.startDirEnum = DIR_UP;
euro.startPixel = {
    x: 16*tileSize-1,
    y: 17*tileSize + midTile.y,
};
...
euro.startMode = COIN_PACING_HOME;
euro.arriveHomeMode = COIN_PACING_HOME;

cloud.startDirEnum = DIR_LEFT;
cloud.startPixel = {
    x: 14*tileSize-1,
...
var getActColor = function(act) {
    if (gameMode == GAME_CLOUD) {
        return {
            wallFillColor: mapcloud.wallFillColor,
            wallStrokeColor: mapcloud.wallStrokeColor,
            pelletColor: mapcloud.pelletColor,
        };
    }
};
